...
function closeDialog(dialog) {
  ...
  try {
    ...
    console.warn("The dialog could not be closed normally.", error);
  }

  // Fallback for browsers that leave the dialog visible.
...
function renderDashboard() {
  ...
  if (!employee) {
    ...
    els.deleteEmployee.disabled = true;
    activeLeaveFilter = null;
    updateFilterUi();
  ...
  els.deleteEmployee.disabled = false;
  ...
  const visibleEntries = activeLeaveFilter
    ? employeeEntries.filter((entry) => entry.type === activeLeaveFilter)
    : employeeEntries;
...
function updateFilterUi() {
  document.querySelectorAll("[data-leave-filter]").forEach((card) => {
    const isActive = card.dataset.leaveFilter === activeLeaveFilter;
    card.classList.toggle("filter-active", isActive);
    card.setAttribute("aria-pressed", String(isActive));
  });

  els.removeFilter.hidden = !activeLeaveFilter;
  els.removeFilter.textContent = activeLeaveFilter ? `Remove ${activeLeaveFilter.toLowerCase()} filter` : "Remove filter";
}

function setLeaveFilter(filter) {
  activeLeaveFilter = activeLeaveFilter === filter ? null : filter;
  renderDashboard();
}
...
document.querySelectorAll("[data-leave-filter]").forEach((card) => {
  card.addEventListener("click", () => setLeaveFilter(card.dataset.leaveFilter));
  card.addEventListener("keydown", (event) => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      setLeaveFilter(card.dataset.leaveFilter);
    }
  });
});

els.removeFilter.addEventListener("click", () => {
  activeLeaveFilter = null;
  renderDashboard();
});
...
els.deleteEmployee.addEventListener("click", async () => {
  const employee = employees.find((item) => item.id === selectedEmployeeId);
  if (!employee) return;

  const employeeLeaveEntries = leaveEntries.filter((entry) => entry.employeeId === employee.id);
  const message = employeeLeaveEntries.length
    ? `Delete ${employee.name} and ${employeeLeaveEntries.length} associated leave ${employeeLeaveEntries.length === 1 ? "entry" : "entries"}? This cannot be undone.`
  ...
  try {
    await Promise.all([
      ...employeeLeaveEntries.map((entry) => deleteDoc(doc(db, "leaveEntries", entry.id))),
    ...
    console.error(error);
    els.deleteEmployee.disabled = false;
    showToast("Could not delete the employee profile.", true);
  }
});
...
els.employeeForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    closeDialog(els.employeeDialog);
...
els.leaveForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const submitButton = els.leaveForm.querySelector('button[type="submit"]');
  const employee = employees.find((item) => item.id === selectedEmployeeId);
  if (!employee) return;
  ...
  try {
    ...
    els.leaveForm.reset();
    document.querySelector("#leaveType").value = "Vacation";
    document.querySelector("#leaveStatus").value = "Approved";
    closeDialog(els.leaveDialog);
    ...
    submitButton.textContent = "Save Leave";
  }
});
